...
function renderProfileSection(profile, repos) {
  ...
  return [
    ...
    "",
    `<sub>Auto synced at ${new Date().toISOString().replace("T", " ").slice(0, 16)} UTC</sub>`,
  ].join("\n");
}
...
function renderHeroStats(profile, repoCount, totalStars, totalForks) {
  ...
  return [
    "<div align=\"center\">",
    "",
    `<img src=\"${profileViews}\" alt=\"profile views\" />`,
    "",
    `<img src=\"${statsUrl}\" alt=\"GitHub stats\" height=\"165\" />`,
    `<img src=\"${streakUrl}\" alt=\"GitHub streak\" height=\"165\" />`,
    "",
    `<img src=\"${trophyUrl}\" alt=\"GitHub trophies\" width=\"100%\" />`,
    "",
    "</div>",
    "",
    "<table>",
    "  <tr>",
    metricCell("Repositories", repoCount, "22d3ee"),
    metricCell("Project Stars", totalStars, "ec4899"),
    metricCell("Project Forks", totalForks, "8b5cf6"),
    metricCell("Followers", profile.followers, "22c55e"),
    "  </tr>",
    "</table>",
    "",
  ].join("\n");
...
function renderActivityGraph() {
  ...
  return [
    "### Activity Pulse",
    "",
    `<img src=\"https://github-readme-activity-graph.vercel.app/graph?username=${encodedOwner}&theme=tokyo-night&hide_border=true&area=true&bg_color=0d1117&color=22d3ee&line=ec4899&point=ffffff\" alt=\"GitHub activity graph\" width=\"100%\" />`,
    "",
  ].join("\n");
}
...
function renderLanguageBadges(languages) {
  ...
  return ["### Language Matrix", "", "<p align=\"center\">", badges, "</p>", ""].join("\n");
}

function renderRepoGrid(repos) {
  if (!repos.length) {
    return "> No repositories to display yet.";
  }
...
function renderRepoCard(repo) {
  ...
  return [
    ...
    `      <a href=\"${repo.html_url}\"><img src=\"${cardUrl}\" alt=\"${escapeHtml(repo.name)}\" width=\"100%\" /></a>`,
    `      <br /><sub>${description}</sub>`,
    `      <br /><sub>Stars ${repo.stargazers_count} · Forks ${repo.forks_count} · ${repo.language || "Code"}</sub>`,
    "    </td>",
  ].join("\n");
}
